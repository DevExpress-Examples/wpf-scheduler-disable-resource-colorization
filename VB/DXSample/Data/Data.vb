...
Namespace DXSample.Data
	...
	Public Class SportEvent
		...
		Public Property Caption() As String
			...
			Set(ByVal value As String)
				SetValue(value)
				RaisePropertyChanged(Function() CustomText)
			End Set
		End Property
		...
		Public ReadOnly Property CustomText() As String
			Get
	...
	Public Class SportChannel
		...
		Public Property NewsTimeRange() As TimeSpanRange
			Get
				Return GetValue(Of TimeSpanRange)()
			End Get
			Set(ByVal value As TimeSpanRange)
				SetValue(value)
				RaisePropertyChanged(Function() HasNews)
			End Set
		End Property
	End Class
	...
	Public Class SportChannelsDataHelper
		Public Shared ReadOnly SportGroups() As SportGroup = {
			...
			New SportGroup() With {
				.Id = 1,
				.Caption = "Basketball",
				.Color = Color.FromRgb(&HFF, &HC2, &HBE)
			},
			New SportGroup() With {
				.Id = 2,
				.Caption = "Boxing",
				.Color = Color.FromRgb(&HA8, &HD5, &HFF)
			},
			New SportGroup() With {
				.Id = 3,
				.Caption = "Tennis",
				.Color = Color.FromRgb(&HC1, &HF4, &H9C)
			},
			New SportGroup() With {
				.Id = 4,
				.Caption = "Soccer",
				.Color = Color.FromRgb(&HF3, &HE4, &HC7)
			},
			New SportGroup() With {
				.Id = 5,
				.Caption = "Artistic Gymnastics",
				.Color = Color.FromRgb(&HF4, &HCE, &H93)
			},
			New SportGroup() With {
				.Id = 6,
				.Caption = "Canoe",
				.Color = Color.FromRgb(&HC7, &HF4, &HFF)
			},
			New SportGroup() With {
				.Id = 7,
				.Caption = "Kayak",
				.Color = Color.FromRgb(&HCF, &HDB, &H98)
			},
			New SportGroup() With {
				.Id = 8,
				.Caption = "Sailing",
				.Color = Color.FromRgb(&HE0, &HCF, &HE9)
			},
			New SportGroup() With {
				.Id = 9,
				.Caption = "Swimming",
				.Color = Color.FromRgb(&H8D, &HE9, &HDF)
			},
			New SportGroup() With {
				.Id = 10,
				.Caption = "Shooting",
				.Color = Color.FromRgb(&HFF, &HF7, &HA5)
			}
		}
		...
		Private Shared Function GenerateEvents(ByVal dayCount As Integer) As ObservableCollection(Of SportEvent)
			...
			For Each ch As SportChannel In sportChannels
				Dim start As DateTime = DateTime.Today.Subtract(TimeSpan.FromDays(dayCount))
				Dim [end] As DateTime = DateTime.Today.Add(TimeSpan.FromDays(dayCount))
				If ch.HasNews Then
'INSTANT VB WARNING: An assignment within expression was extracted from the following statement:
'ORIGINAL LINE: result.Add(GetRandomRecurrenceSportEvent(aptId++, start, ch, (dayCount * 2) - 1));
...
'INSTANT VB WARNING: An assignment within expression was extracted from the following statement:
'ORIGINAL LINE: result.Add(GetRandomSportEvent(aptId++, i, i + TimeSpan.FromDays(1), ch.Id));
						result.Add(GetRandomSportEvent(aptId, i, i.Add(TimeSpan.FromDays(1)), ch.Id))
						aptId += 1
						i = i.Add(TimeSpan.FromDays(1))
						Continue Do
					End If
					Dim airtimeStart As New DateTime(i.Year, i.Month, i.Day, 6, 0, 0)
					Dim airtimeEnd As New DateTime(i.Year, i.Month, i.Day, 21, 0, 0)
					Dim aptStartTime As DateTime = airtimeStart
					Do While aptStartTime < airtimeEnd
						Dim aptEndTime As DateTime
...
'INSTANT VB WARNING: An assignment within expression was extracted from the following statement:
'ORIGINAL LINE: result.Add(GetRandomSportEvent(aptId++, aptStartTime, aptEndTime, ch.Id));
...
'ORIGINAL LINE: result.Add(GetRandomSportEvent(aptId++, aptStartTime, aptEndTime, ch.Id));
		...
		Private Shared Function GetRandomRecurrenceSportEvent(ByVal id As Integer, ByVal startDate As DateTime, ByVal channel As SportChannel, ByVal dayCount As Integer) As SportEvent
			...
			pattern.Type = CInt(Math.Truncate(AppointmentType.Pattern))
			...
			pattern.RecurrenceInfo = String.Format(CultureInfo.InvariantCulture, recurrenceInfoFormat, recInfo.Start, recInfo.End, CInt(Math.Truncate(recInfo.WeekDays)), recInfo.OccurrenceCount, CInt(Math.Truncate(recInfo.Range)), CInt(Math.Truncate(recInfo.Type)), recInfo.Id.ToString())
			Return pattern
		End Function
		Private Shared Function GetRandomSportEvent(ByVal id As Integer, ByVal start As DateTime, ByVal [end] As DateTime, ByVal channelId As Integer) As SportEvent
...
'INSTANT VB NOTE: The variable events was renamed since Visual Basic does not handle local variables named the same as class members well:
		...
		Private Shared Function GetRandomNewsTimeRange(ByVal start As TimeSpan) As TimeSpanRange
			Return New TimeSpanRange(start, start.Add(TimeSpan.FromMinutes(rnd.Next(30, 60))))
		End Function
		#Region "Sample Data"
		...
		Private Shared ReadOnly sportChannels() As SportChannel = {
			New SportChannel() With {
				.Id = 0,
				.Caption = "SPORT TV 1",
				.NewsTimeRange = GetRandomNewsTimeRange(TimeSpan.FromHours(11))
			},
			New SportChannel() With {
				.Id = 1,
				.Caption = "SPORT TV 2",
				.NewsTimeRange = GetRandomNewsTimeRange(TimeSpan.FromHours(19))
			},
			New SportChannel() With {
				.Id = 2,
				.Caption = "SPORT TV 3"
